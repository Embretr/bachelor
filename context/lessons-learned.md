...
- **Any sentence operationalising Trust/control authority** → use a single plainer verb ("review", "review and override", "approve or change"), never the four-word list "inspect, modify, accept, or reject".
...
- **Rule:** Each thesis chapter (Ch 2 through Ch 6) carries a short opening paragraph immediately under its `\chapter{}` line and above its first `\section{}`. The opening paragraph (a) names the foundations / themes / claims the chapter establishes, (b) signals asymmetric depth where one section dominates, (c) flags content referenced only to delimit (e.g. VRP and TMS in Ch 2, sustainability frameworks in Ch 5), (d) names the anchors the chapter preloads or carries (Efficiency / Trust/control / Adaptability where structurally relevant). Four to six sentences is the right length: long enough to map the chapter, short enough not to become a second introduction. Ch 1 is exempt (its §1.1 already does this work for the whole thesis).
...
### Don't list the four Trust/control actions; use a plainer verb instead
...
- **Why:** Supervisor 2026-05-06 plain-prose calibration on Ch 1 + user directive 2026-05-06 ("dont ever do this list inspects, modifies, accepts, or rejects, could always be explained easier"). Supersedes Sub-clause C of *Bind artefact elements to anchors with transparent verbs* (added 2026-05-04, struck 2026-05-06). The earlier rule answered the failure of replacing the four with a vague count; following that rule produced the new failure of recitation as crutch. The replacement single verb still binds the anchor element to Trust/control; it does so without the litany.
- **When to apply:** Every section, every chapter where Trust/control is operationalised in prose. Particularly §1.2 Trust/control anchor, §1.3 SQ2 (question + explanation), §1.4 Scope, §3.1, §3.2 DSRM Applied "Objectives" bullet, §4.4–§4.5 system features, §5.1.2 Trust/control discussion, §6.2 SQ2 RQ-answer paragraph. Reviewers must flag any reappearance of the four-action list as a critical issue.
