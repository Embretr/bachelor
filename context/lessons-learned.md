# Lessons Learned — Operative Voice of the Thesis

> **Read this file before any revision pass.**
> Patterns here are distilled from past supervisor directives, per-paragraph feedback, and review rounds — they are what the thesis has decided its prose should be. Every revision must honour them; a new section that violates a rule is a regression even if no individual sentence is wrong.
>
> **How to add an entry.** When supervisor feedback or per-paragraph feedback names a pattern that is general (not section-specific), add a rule. Keep entries short: one rule body, one short **Why**, one **When to apply**. Source the rule by attributing it to the supervisor-log entry, paragraph hash, or review round that produced it. After adding the rule body, also add it to the index below under the chapter/section scopes it touches.
>
...
>
> **Section-specific findings** (a one-off observation about exactly one paragraph) stay in `feedback-app/state.json` rather than here. Only generalisable patterns belong in this file.
...
- **Any sentence operationalising Control authority** → use a single plainer verb ("review", "review and override", "approve or change"), never the four-word list "inspect, modify, accept, or reject".
...
### Don't claim TA themes "emerged" or "surfaced unaided" — name them as products of the coding process
- **Rule:** When prose describes the outputs of a thematic analysis, do NOT write "themes emerged from the data", "themes surfaced", "themes that arose from the interviews", or any passive-emergence formulation. Themes in Braun & Clarke's six-phase TA are constructed through the analyst's coding decisions, not discovered as pre-existing properties of the transcript. The required active framing is "themes were named through the coding process", "the analysis identified X themes", "five themes were constructed across the seven transcripts", or any phrasing that puts the analyst's coding work as the cause of the theme. Anchor the active framing to Braun & Clarke (2006) Table 2 criterion 15 ("themes are not allowed to 'emerge'") at first naming if the section needs the explicit defence.
- **Why:** §3.4 round 1 quality review flagged this as a forbidden-phrase pattern. "Themes emerged" reads to a sensor as inductive name-dropping without inductive practice — it claims the TA mode without owning the coding work that produces themes. The forbidden phrase is also a frequent AI-voice tell because it lets the writer avoid stating who did what.
- **When to apply:** §3.4 (Data Analysis), §4.1 (Findings — TA-output paragraphs), §5.1.x (Discussion synthesising TA outputs), §5.6 (Methodology Reflection if it revisits the analysis stance), §6.2 if Ch 6 summarises findings as "themes that emerged".
- **Source:** §3.4 round 1 quality review (2026-05-03), called out as model phrasing.

### Inductive-coding language must be consistent across sections — no slip to "hypothesis" downstream
- **Rule:** If a method section commits the thesis to inductive analytic positioning (e.g. §3.4 ¶1: "the visibility-gap label was named only after coding"), downstream sections must NOT refer back to the same finding as "the hypothesis about X", "the prior assumption", "the expectation we tested", or any pre-formed-claim language. The two framings collide and a sensor reading them side by side sees the seam. The fix is a vocabulary lock: use "prior expectation", "the framing carried into the analysis", or "what the analysis named" rather than "hypothesis" or "assumption". Reserve "hypothesis" for sections where a hypothesis was actually formed and tested deductively (this thesis has none).
- **Why:** Quality-ch3 round 3 (2026-05-03) flagged §3.7 ¶3 saying "the hypothesis about the visibility gap" while §3.4 had already committed to "named only after coding". The wording was a one-word inconsistency but it pulls against the inductive position the chapter spent ¶1 establishing. Cross-section consistency for this exact word is a sensor-grade discipline check.
- **When to apply:** §3.7 reflexivity, §5.1.x (Discussion sub-sections re-citing TA outputs), §5.6 (Methodology Reflection), §6.1–§6.2 (Conclusion narrating the empirical contribution). Any time a Ch 4 finding is referenced in framing prose.
- **Source:** Chapter-3 sensor review round 3 (2026-05-03).

### Every chapter opens with a 4–6 sentence framing paragraph above its first numbered section
- **Rule:** Each thesis chapter (Ch 2 through Ch 6) carries a short opening paragraph immediately under its `\chapter{}` line and above its first `\section{}`. The opening paragraph (a) names the foundations / themes / claims the chapter establishes, (b) signals asymmetric depth where one section dominates, (c) flags content referenced only to delimit (e.g. VRP and TMS in Ch 2, sustainability frameworks in Ch 5), (d) names the anchors the chapter preloads or carries (Efficiency / Control / Adaptability where structurally relevant). Four to six sentences is the right length: long enough to map the chapter, short enough not to become a second introduction. Ch 1 is exempt (its §1.1 already does this work for the whole thesis).
- **Why:** Redthread-ch2 + quality-ch2 (2026-05-02) both named the missing chapter-intro paragraph as the single most important fix, ahead of every section-level issue. Without the intro, the reader enters at §X.1's level and has to reverse-engineer what the chapter is for. The intro is also the chapter's most legible anchor-spine commitment to a sensor reading top-down.
- **When to apply:** Ch 2 (model intro currently absent — TODO comment); Ch 3 (brief intro present, should be confirmed at 4–6 sentences and named-anchor); Ch 4, Ch 5, Ch 6 (must be added at chapter-write time, not deferred to chapter-review).
- **Source:** Redthread-ch2 + quality-ch2 (2026-05-02).
...
- **Source:** Supervisor 2026-05-02 (logged in `context/supervisor-log.md`).
...
- **When to apply:** §1.1 (introduction of the demand), §3.1 (Defining the Task), §4.1 (interview themes on the artefact's relevance), §5.1.2 (Control discussion of operator-vs-owner asymmetry).
- **Source:** User directive 2026-05-04, Ch 1 §1.1 review.
...
- **Source:** Supervisor 2026-05-04 (logged in `context/supervisor-log.md`).
...
- **Source:** §3.2 round 1 coherence review (2026-05-03); reinforced + measures->metrics added by supervisor 2026-05-04 (logged in `context/supervisor-log.md`).
...
- **When to apply:** Any prose section, especially Ch 2 background, Ch 4.3 fit/gap, Ch 5 discussion. The internal labels remain free to use in working notes outside `result/` (e.g. paragraph-feedback drafts, supervisor-log entries) but never in the thesis prose itself.
- **Source:** Supervisor 2026-05-02 (logged in `context/supervisor-log.md`).

### Hard rule: abbreviate Ressursplanlegger as RP after first mention
- **Rule:** The artefact name **Ressursplanlegger** is defined exactly once at its first prose mention in the thesis as `Ressursplanlegger (hereafter RP)`, and the abbreviation **RP** is used in every subsequent body-prose mention thereafter. The first mention is currently in Ch 2 §preamble; it will move to Ch 1 once that chapter is drafted, at which point the Ch 2 expansion is removed. Section labels (e.g. `sec:ressursplanlegger`), code identifiers, file paths (`/ressursplanlegger`), and the title page keep the full name. Context files (`context/glossary.md`, `context/thesis-spine.md`) keep the full name as the canonical glossary term.
...
- **Source:** User directive 2026-05-04 (logged in `context/supervisor-log.md` if persisted; preserved here as the enforceable rule for writers/reviewers).
...
### Don't list the four Control actions; use a plainer verb instead
- **Rule:** Do not enumerate "inspect, modify, accept, or reject" in any prose. Use a single, plainer verb that captures the same authority — "review", "review and override", "approve or change", "have the final say on" — chosen to fit the sentence. The four-action recital reads as a stylistic crutch and forces the reader to parse a litany every time the concept appears; the supervisor's example sentence lands the same idea in one verb: "the coordinator must be able to review every algorithmic assignment before it takes effect, since trust requires oversight." A reader who has met the concept once does not need it re-spelled every time.
  - BEFORE: "The coordinator inspects, modifies, accepts, or rejects every plan the algorithm proposes before it takes effect."
  - AFTER: "Every plan the algorithm proposes is reviewed by the coordinator before it takes effect."
  - BEFORE: "Through which interface and process mechanisms can the traffic coordinator inspect, modify, accept, or reject every algorithm-generated assignment?"
  - AFTER: "Through what kind of interface and process can the traffic coordinator review and override every algorithm-generated assignment?"
- **Why:** Supervisor 2026-05-06 plain-prose calibration on Ch 1 + user directive 2026-05-06 ("dont ever do this list inspects, modifies, accepts, or rejects, could always be explained easier"). Supersedes Sub-clause C of *Bind artefact elements to anchors with transparent verbs* (added 2026-05-04, struck 2026-05-06). The earlier rule answered the failure of replacing the four with a vague count; following that rule produced the new failure of recitation as crutch. The replacement single verb still binds the anchor element to Control; it does so without the litany.
- **When to apply:** Every section, every chapter where Control is operationalised in prose. Particularly §1.2 Control anchor, §1.3 SQ2 (question + explanation), §1.4 Scope, §3.1, §3.2 DSRM Applied "Objectives" bullet, §4.4–§4.5 system features, §5.1.2 Control discussion, §6.2 SQ2 RQ-answer paragraph. Reviewers must flag any reappearance of the four-action list as a critical issue.
- **Source:** Supervisor 2026-05-06 + user directive 2026-05-06 (Ch 1 §1.2.2 review).
