...
The DSR process follows Peffers et al. (2007) six activities, applied to *this* project rather than discussed in the abstract. Each row maps an activity to project-specific content + the anchor it serves (Efficiency / Trust/control / Adaptability).

| DSRM Activity | What was done | Anchor |
|---|---|---|
| **1. Problem Identification and Motivation** | Brief semi-structured interviews (~15 min each) with seven traffic coordinators surfaced the *resource-utilization visibility gap* and the *operator-vs-owner asymmetry*. Admmit's bachelor task brief established the HITL principle and multi-tenant scope from the start. | Efficiency (gap) + Trust/control (HITL mandate) |
| **2. Objectives of a Solution** | Locked anchor concepts as the operational objectives — Efficiency (overtime, idle time, load balance), Trust/control (inspect / modify / accept / reject — the four operator actions on every algorithm-generated assignment), Adaptability (cross-company adaptability via configurable weights and rules). Functional + non-functional requirements derived through MoSCoW + interview-source traceability. | All three |
