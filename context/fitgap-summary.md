...
**Systems evaluated:** Timpex, Opter, custom/manual systems (as used by the 7 interviewed companies)

> **Note:** Neither Timpex nor Opter generates assignment plans automatically. Both are order-management and invoicing systems; driver-to-assignment matching is performed manually by the traffic coordinator in every interviewed company that uses them.
...
|---|------|-------------------------------|--------|:--------:|
| F-01 | Order/assignment registration | Both systems allow creating and storing transport orders | Timpex, Opter | Full |
| F-02 | Invoicing and billing | Both commercial systems handle invoicing — identified by interviewees as the primary function of their current TMS | Timpex, Opter | Full |
| F-03 | Driver notification | Both systems have driver-facing apps (Timpex Confirm, Opter order app) | Timpex, Opter | Partial |
| F-04 | Basic driver/vehicle data storage | All systems store some driver and vehicle information | All | Partial |
| F-05 | Assignment status tracking | Orders can be marked as completed for invoicing purposes | Timpex, Opter | Partial |
...
| G-02 | **Algorithm-generated plan** — automatic assignment of drivers/vehicles to assignments | Neither Timpex nor Opter offers optimisation or automated plan generation. All driver-to-assignment matching is performed manually. | **High** | FK-03 |
...
| G-05 | **Competency and certification tracking** — store licence classes, ADR, crane certs with expiry dates | Timpex and Opter store basic driver data but do not track competencies or certifications with expiry. | **High** | FK-09, FK-10 |
...
| G-14 | **System performance** — fast, responsive interface with concurrent users | Timpex is described as extremely slow when multiple users are logged in. Performance is a major pain point. | **High** | (non-functional) |
...
|--------|---------|-------------------|---------------|
| **Timpex** | Norlog Lakselv, Norlog Mo i Rana, Norlog Tana | Order management, invoicing, driver notification (Confirm app) | No planning, no optimisation, extremely slow, no capacity overview, no conflict detection |
| **Opter** | Harlem Solutions | Order management, invoicing, driver app | Primarily a billing tool; no planning support at all |
...
| Capability | Timpex | Opter | Custom | None | Ressursplanlegger |
|-----------|:------:|:-----:|:------:|:----:|:-----------------:|
...
| Competency tracking | ❌ | ❌ | ❌ | ❌ | ✅ |
| Manual override | N/A | N/A | N/A | N/A | ✅ |
...
1. **Ressursplanlegger does not replace existing TMS** — it fills the planning gap. Companies would still need Timpex or Opter for invoicing.
