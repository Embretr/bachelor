...
**Efficiency**
Improved resource utilisation across three concrete dimensions: reduced overtime, reduced idle time between assignments, and reduced uneven load between drivers (load balancing). Secondary: reduced time spent on the planning activity itself. Visibility into current utilisation is the precondition for optimization — the system's primary value is making invisible utilisation patterns legible to the coordinator and the company.
...
**Visibility gap**
The absence of a structured, comparable overview of resource utilisation in current Norwegian transport-company planning practice. Overtime is handled reactively, idle time is invisible, and load balancing depends on coordinator memory. The visibility gap is the empirical starting point of the thesis argument and the precondition that makes optimization a meaningful goal.

**Operator-vs-owner asymmetry (demand asymmetry)**
The structural pattern in which the parties who specify and pay for an algorithm-assisted system are not the same as the parties who must run it day to day. Theoretical anchor: Bainbridge (1983), *Ironies of Automation*. The thesis applies the term to one observed configuration (Admmit and the transport-company owners articulate a demand for utilisation optimization that the operating coordinators do not articulate in the same words), but not to a unanimous one — several interviewed coordinators welcomed an algorithm that proposes assignments, and the asymmetry refers to the structural pattern rather than a unanimous opposition. Use the term verbatim where it appears; do not paraphrase as "stakeholder mismatch", "user-vs-buyer gap", or similar. Note that under the 2026-05-06 RQ reframe, the term continues to appear in §2.2 (Bainbridge) and §5.1, but is no longer the central framing of the thesis — that role now sits with the automation/utility balance the RQ states directly.
