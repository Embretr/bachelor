...
3. **Chapter 5 sub-section anchor structure.** For sections under §5.1 (anchor-organised primary findings — §5.1.1 Efficiency, §5.1.2 Trust/control, §5.1.3 Adaptability), the section must have at least one `MUST ANCHOR` marker tied to **exactly one** locked anchor whose name matches the sub-section's intended anchor (e.g. §5.1.1 → Efficiency only). If a §5.1.x section has no `MUST ANCHOR` to a locked name, or anchors to two anchors at once, **STOP**.
