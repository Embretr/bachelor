...
A constraint solver solves a different version of the same problem. OR-Tools CP-SAT searches the assignment space systematically with explicit hard and soft constraints, and returns a near-optimal or optimal plan within a time budget the team could set explicitly. On the small and medium benchmark instances (sizes given in \Cref{sec:evaluation-framework}), CP-SAT scheduled more assignments than greedy and proved optimality. On the large instance the time budget bound the search: CP-SAT did not finish within the 120-second cap.
...
The framework that tests RP's claims is asymmetric on purpose. Some claims are tested directly through the multi-engine benchmark, the auto-router run history, and the requirements traceability matrix. Others are accepted as inputs the framework does not itself test, following the position carried through \Cref{sec:solver-layer}. Two such accepted-as-input claims anchor the artefact. The first is that the visibility gap is real, which the seven coordinator interviews surfaced (\Cref{sec:interview-recruitment}). The second is that human-in-the-loop operation is the right operator stance, which Admmit fixed in the project's first week (\Cref{sec:defining-task}). The framework tests something else: whether solver approaches behave differently under the same constraint combinations, whether the auto-router has enough logged evidence to choose one engine instead of racing the full pool, and whether the implemented artefact covers the requirements. It does not ask whether the problem is worth solving. The validate-versus-evaluate distinction that bounds these claims is set out under DSR theory in \Cref{sec:dsr} and is not repeated here.

\subsection{Multi-engine benchmark protocol}
\label{sec:eval-benchmark}

Three synthetic datasets feed the benchmark, with sizes chosen to span the fleet ranges named in \Cref{sec:interview-recruitment}. The small dataset has five employees, three vehicles, and twenty assignments. It is a sanity-scale instance below the smallest interviewed fleet. The medium dataset has twenty employees, ten vehicles, and one hundred assignments. It sits at the small-operator end of the interviewed range and exercises overlap and workload-balance terms more heavily. The large dataset has fifty employees, twenty-five vehicles, and five hundred assignments. It exceeds the largest multi-department fleet observed and is a stress-scale instance designed to expose where exact search begins to bind. Synthetic data was used rather than production data for two concrete reasons, not from methodological preference. First, no transport company has operated the artefact, so production data does not exist for this codebase. Second, the operational data the seven interviewed companies hold falls under the data-protection bounds the consultation framing in \Cref{sec:ethics} chose not to cross. The price the framework pays for that choice is that benchmark numbers cannot be projected onto real Norwegian-transport operational data without further validation, named as L5 in \Cref{sec:limitations}.

The run protocol is held constant across engines so that the comparison is on solver behaviour rather than on benchmark configuration. The datasets are generated deterministically with seed 42, and the generator clamps assignment requirements against the available employee and vehicle pools. This keeps the benchmark focused on real resource scarcity and overlapping time windows rather than on impossible requests created by the generator. Each (engine, dataset) pairing is executed once, producing a single-sample wall-clock figure that the analysis labels honestly as such throughout. The engines receive the same problem JSON, the same shared constraint configuration, and the same canonical scorer after the run. The metrics logged per run are scheduled-assignment count and percentage, optimality where the engine can prove it, and wall-clock solve time, alongside success or timeout.
