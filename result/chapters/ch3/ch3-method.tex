...
Admmit fixed three requirements before any code was written. The platform operates as Human-in-the-Loop (HITL): the algorithm proposes a plan, and the coordinator has the final say on every assignment. The platform is multi-tenant: one installation serves multiple customer companies with each company's data kept apart. And every plan respects working-hours and rest-period rules from the Norwegian Working Environment Act (\textit{Arbeidsmiljøloven}) as hard constraints.

The brief fixed the topic but left the empirical foundation open. Two questions needed answers first: how do Norwegian traffic coordinators actually plan, and which software, if any, do they use today? Neither could be answered from by us. Drawing on Admmit's customer roster, the team ran semi-structured phone calls with traffic coordinators and transport managers over the following month. From then on, the interviews and the artefact-building influenced each other throughout the project.

% Sjekket: temasetninger står alene i hver paragraf, 3 krav er den eneste lista (sentral), ingen em-dash, ingen overkonkret påstand, HITL og multi-tenant glosset inline, hver setning bærer info, fakta verifisert (Admmit, NTNU, RP-navn, 3 krav fra brukeren).
...
% TODO: Etter kodefiksen bør denne seksjonen oppdateres til faktisk
% koordinator-språk: rask, middels og grundig. Forklar at koordinator velger
% tid/kvalitet, mens systemet velger engine via historikkbasert auto-router.
...
% TODO: Etter kodefiksen bør denne seksjonen styrkes: soft-constraint weights
% skal beskrives som persistent per-company settings som brukes som default i
% solve-flowen, ikke bare som en delvis realisert konfigurasjonsmekanisme.
...
% TODO: Når solveAuto er wired inn i normal solve-flow, gjør denne TODO-blokken
% om til vanlig metodeprosa. Beskriv cold start, warm path, size bucket,
% category og companyId som evaluerbar mekanisme.

\section{Evaluation Framework}
\label{sec:evaluation-framework}

% TODO: Etter kodefiksen bør Evaluation Framework utvides fra to til tre
% instrumenter: multi-engine benchmark, requirements traceability og
% auto-router run history. Avgrens auto-router-loggen som teknisk validering av
% enginevalg, ikke som bevis på produksjonsgevinst.
...
% TODO: Skriv dette som vanlig validity/reliability-tekst etter kodefiksen.
% Poeng: auto-routerens reliabilitet avhenger av nok historikk per
% company/category/size bucket, fravær av skjev historikk og stabile
% engine-versjoner.
