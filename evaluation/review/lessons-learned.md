...
- **Long prose paragraphs (>100 words, non-bulleted)** → em-dash density cap (≤2 per 100 words).
...
- **Rule:** When a sentence ties a concrete artefact element (a UI feature, an architectural property, an algorithmic capability) to one of the three locked anchor names, use a functional verb — `enables`, `realises`, `implements`, `operationalises`, `delivers` — not a metaphor — `is the substrate of`, `is the foundation of`, `is the scaffolding for`, `embodies the spirit of`. The functional verb names the causal relationship; the metaphor only hints at it. If the verb needs qualification (how it enables, by what mechanism), add a `by ...` clause rather than reaching for a metaphor.
  - **Sub-clause A — anchors cannot be the grammatical subject of imperative verbs.** The anchor name itself must NOT be the grammatical subject of `requires`, `demands`, `mandates`, or any other imperative verb. The artefact element is the subject; the anchor is the realised property. BEFORE: "Effektivitet requires that the artefact reduce overtime"; AFTER: "Effektivitet is realised by the artefact reducing overtime". This rule applies inside relative clauses too: "the configurability surface that Tilpasningsdyktighet requires" inverts the binding — rewrite as "the configurability surface through which Tilpasningsdyktighet is delivered".
  - **Sub-clause B — when describing what was decided/built/fixed, name the artefact element, not the anchor.** Where §3.1 has carefully separated the artefact element (HITL, multi-tenant architecture) from the anchor it operationalises (Tillit/kontroll, Tilpasningsdyktighet), downstream paragraphs must preserve that distinction. BEFORE: "Admmit's mandate fixed Tillit/kontroll and Tilpasningsdyktighet"; AFTER: "Admmit's mandate fixed HITL and the multi-tenant architecture (the artefact elements operationalising Tillit/kontroll and Tilpasningsdyktighet)". The anchors are the realised properties, not the things Admmit decided.
- **Why:** Anchor-binding sentences are spine-load-bearing across §1.2, §2.2 (HITL), §3.1, §4.4, §4.5, every §5.1.x sub-section, §6.2. A metaphorical bind reads as decorative compression to a sensor reading for argumentative discipline; the same sentence with `enables` is judged transparent. Example from §3.1 round 1: BEFORE — "the multi-tenant architecture is the technical substrate of Tilpasningsdyktighet"; AFTER — "the multi-tenant architecture enables Tilpasningsdyktighet by isolating per-customer configuration in a single deployed instance". Sub-clauses A and B emerged from §3.2 round 1 + round 2: the same anchor-binding category includes inverted constructions and artefact-element-vs-anchor-property collapse, both of which a strict sensor would flag.
- **When to apply:** Any sentence in any chapter that names a locked anchor (Effektivitet, Tillit/kontroll, Tilpasningsdyktighet) and ties an artefact element to it. Particularly §1.2 (anchor definitions), §3.1 ¶4 (origin → anchor binding), §3.2 (DSRM Applied "Objectives" bullet + closing-paragraph project-ordering caveat), §4.4–4.5 (system features → anchor), §5.1.1/§5.1.2/§5.1.3 (every sub-section), §6.2 (RQ-answer paragraphs).
...
### Em-dash budget for narrative sections with bulleted iteration / finding / decision structure
- **Rule:** Sections built on a six-bullet iteration / finding / decision structure (currently §3.5; anticipated §4.5 multi-engine benchmark, §4.7 process documentation, §5.1.1/§5.1.2/§5.1.3, §5.4 L#) cap parenthetical em-dashes (`---`) at one per sub-subsection. The recurring `X --- a Y` / `X --- the Z` construction is a strong AI-voice tic that reads as decoration rather than as syntactic necessity. Replace with commas (apposition: "configurable time budget, a property that matters"), colons (orientation: "windows: week boundaries and mid-shift absences"), parentheses (true aside), or a sentence break. Matched-pair em-dashes (`X --- the parenthetical-aside --- continued`) follow the supervisor-sanctioned fit/gap-style use and are not under this cap; lone trailing em-dashes (`X --- the elaboration`) are. Acronym-expansion em-dashes (`CP-SAT (Constraint Programming --- Satisfiability)`) are noise next to the surrounding parens and should drop the dash entirely.
- **Why:** §3.5 round 1 had 7 lone parenthetical em-dashes across 8 sub-subsections — visible to a sensor reading for naturalness as a stylistic tic. The pattern recurs because each iteration's "Why" / "Learned" bullet naturally invites apposition, and the writer reaches for `---` rather than the lighter alternatives. The cap forces the lighter punctuation, which restores sentence rhythm and removes the tic.
- **When to apply:** Any section using bulleted iteration / finding / decision structure. §3.5 (Iterative Development Process) is the canonical case; §4.5 (multi-engine benchmark per-engine bullets), §4.7 (process documentation per-artefact bullets), §5.1.1/§5.1.2/§5.1.3 (anchor-organised primary-finding bullets), §5.4 (L1–L12 limitation paragraphs each as a named-finding bullet) are the upcoming cases.
- **Source:** §3.5 round 1 quality review (2026-05-03).

### Em-dash density cap for non-bulleted prose paragraphs (>2 per 100 words is a tic)
- **Rule:** In non-bulleted prose paragraphs (the bulk of Ch 1, Ch 2, Ch 3 narrative sections, Ch 5 discussion paragraphs, Ch 6 conclusion paragraphs), cap em-dashes (`---`, lone or matched-pair) at 2 per 100 words of paragraph prose. Above that density the construction reads as a stylistic tic even when each individual em-dash is syntactically defensible. Lighter alternatives: parentheses for true asides ("research question (coordinator experience of the visibility gap, not its prevalence)"), commas + adjective phrasing for apposition ("Malterud's bounded sense of applicability ... rather than ..."), or a sentence break. This rule is the prose-paragraph counterpart to the bulleted-section em-dash budget above; the bulleted budget caps lone em-dashes at 1 per sub-subsection, this rule caps total density (lone + matched-pair) in prose.
- **Why:** §3.7 round 1 quality review flagged ¶2 packing three matched-pair em-dashes into a 105-word paragraph (~2.9 per 100 words). The bulleted-section em-dash budget did not apply because §3.7 is straight prose, but the same tic-pattern that motivated the bulleted budget appears in prose at high density. The fix in §3.7 R1 was to convert both matched-pair em-dashes to parentheses + adjective phrasing, dropping density to zero and improving rhythm without changing substance.
- **When to apply:** Any prose paragraph in any chapter. Particularly §1.1, §1.2, §3.7, §4.1–§4.4, §5.2, §5.3, §5.5, §5.6, §6.1–§6.3 (sections that are not built on a bulleted iteration / finding / decision structure). When writing a long prose paragraph with multiple appositions, prefer parentheses or commas for the second and third aside even if the first is em-dashed.
- **Source:** §3.7 round 1 quality review (2026-05-03).
